...
- 物理キーボードの配列を JIS / US から選択（環境ごと。起動中は即時反映）
...
アプリの**設定 → 表示と入力**から、デスクトップの見え方と入力を調整できます。
...
接続した物理キーボードに合わせて、**設定 → 表示と入力 → キーボード配列**から **JIS（日本語 106/109キー）** と **US（英語 101/104キー）** を選べます。US配列のキーボードで既定のJISのまま使うと `@` や `[` などの記号がキー刻印とズレますが、配列を合わせれば刻印どおりに入力できます（`@` の位置が最も分かりやすい目印で、JISは 2 の右上、USは Shift+2 です）。

内蔵X11サーバー（Xorg）の XKB を切り替えて実現しており、`setxkbmap` のmodelとlayout、XFCEの `keyboard-layout` 設定、Fcitx5のレイアウトをまとめて合わせます。設定は**環境ごとに保存**され、**デスクトップ起動中は即時**に反映、**停止中に変更した場合は次回起動時**に適用されます。
